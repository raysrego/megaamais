...
export function useCaixa() {
  ...
  const fetchMovimentacoes = useCallback(async (sessaoId: number) => {
    ...
    try {
      ...
      if (isMounted.current && data) setMovimentacoes(data as CaixaMovimentacao[]);
    } catch (err) {
  ...
  const fetchSessaoAtiva = useCallback(async () => {
    ...
    try {
      ...
      if (isMounted.current) {
        if (data) {
          setSessaoAtiva(prev => (prev?.id === data.id ? prev : data as CaixaSessao));
      ...
      if (isMounted.current) setError('Erro ao carregar dados do caixa.');
    } finally {
  ...
  const abrirCaixa = async (
    ...
    try {
      ...
      // 1. Buscar loja_id na tabela perfis (não em usuarios)
      ...
      if (perfilError || !perfil?.loja_id) {
        throw new Error('Usuário não possui loja vinculada (perfil.loja_id ausente)');
      }

      // 2. Opcional: validar terminal_id_ref se fornecido
      let terminalIdRef = terminalId || null;
      if (terminalIdRef) {
        const { data: term, error: termError } = await supabase
          ...
          .maybeSingle();
        if (termError || !term) {
          console.warn(`Terminal ID ${terminalIdRef} não encontrado, enviando null`);
          terminalIdRef = null;
        }
      }
  ...
  const fecharCaixa = async (
    ...
    try {
      ...
      if (sessaoAtual.status !== 'aberto') throw new Error('Sessão já foi fechada');
      ...
      if (dinheiroEmMaos < 0) dinheiroEmMaos = 0;
        ...
        .subscribe((status, err) => {
          if (status === 'SUBSCRIBED') {
            ...
            reconnectTimeoutRef.current = setTimeout(() => {
              if (isMounted.current && sessaoAtiva) setupChannel();
            }, backoffDelay);
          }
        });
