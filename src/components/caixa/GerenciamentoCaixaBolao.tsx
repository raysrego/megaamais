...
import { Lock, Unlock, DollarSign, Users, TrendingUp, Eye } from 'lucide-react';
...
export function GerenciamentoCaixaBolao() {
    ...
    // Caso seja operador: apenas visualização (não pode abrir/fechar)
    if (isOperador) {
        if (!sessaoAtiva) {
            return (
        ...
        // Operador com sessão ativa – apenas informações
        return (
            <div className="p-6">
                <div className="card-premium p-6 bg-linear-to-br from-blue-500/10 to-transparent border-blue-500/20">
                    ...
                    <div className="grid grid-cols-3 gap-4 mt-4">
                        <div className="p-4 rounded-xl bg-white/5 border border-white/10 text-center">
                            ...
                            <p className="text-2xl font-black">R$ {sessaoAtiva.total_vendido.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
                        </div>
                        <div className="p-4 rounded-xl bg-white/5 border border-white/10 text-center">
                            <p className="text-[10px] font-bold text-muted uppercase">Dinheiro</p>
                            <p className="text-2xl font-black">R$ {sessaoAtiva.total_dinheiro.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
                        </div>
                        <div className="p-4 rounded-xl bg-white/5 border border-white/10 text-center">
                            <p className="text-[10px] font-bold text-muted uppercase">PIX</p>
                            <p className="text-2xl font-black">R$ {sessaoAtiva.total_pix.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
                        </div>
                    </div>
    ...
    // Para gestor, gerente e admin: controle total
        ...
        <div className="p-6">
            ...
            {!sessaoAtiva ? (
                <div className="card p-8 text-center">
                    ...
                    <button className="btn btn-primary w-full max-w-md mx-auto h-14" onClick={handleAbrirCaixa} disabled={isOpening}>
                        <Unlock size={20} />
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex gap-3">
                        <button className="btn btn-ghost flex-1 h-14">Ver Vendas Consolidadas</button>
                        <button className="btn btn-danger flex-1 h-14 font-black" onClick={() => setShowModalFechamento(true)}>
