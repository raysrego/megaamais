...
export function LotteryConsolidatedCard({ jogo, cor, boloes, onClick }: LotteryConsolidatedCardProps) {
    ...
    // Ticket médio (preço médio por cota entre todos os bolões)
    const ticketMedio = boloes.length > 0
        ? boloes.reduce((acc, b) => acc + b.precoVendaCota, 0) / boloes.length
        : 0;

    // Próxima data de sorteio (futura)
    const datasFuturas = boloes
        .map(b => new Date(b.dataSorteio))
        .filter(d => d >= new Date(new Date().setHours(0, 0, 0, 0)))
        .sort((a, b) => a.getTime() - b.getTime());

    const dataProximoSorteio = datasFuturas.length > 0
        ? datasFuturas[0]
        : (boloes.length > 0 ? new Date(boloes[0]?.dataSorteio) : null);
            ...
            <div className="p-5 flex-1 flex flex-col">
                <div className="grid grid-cols-2 gap-3 mb-5">
                    ...
                    <div className="p-3 rounded-xl bg-success/5 border border-success/10 text-center">
                        <div className="text-[0.55rem] text-success font-bold uppercase tracking-widest mb-1">Próximo Sorteio</div>
                        <div className="text-[0.8rem] font-black text-success">
                            {dataProximoSorteio ? dataProximoSorteio.toLocaleDateString('pt-BR') : 'A definir'}
                        </div>
                    </div>
                </div>
