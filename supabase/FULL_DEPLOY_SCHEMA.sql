...
-- Seed básico para garantir que existe uma empresa/loja (evita erros de NOT NULL em loja_id)
INSERT INTO public.empresas (id, nome_fantasia, razao_social, cnpj, cidade, uf, ativo)
VALUES ('00000000-0000-0000-0000-000000000001', 'Loja Principal', 'Razão Social Padrão', '00.000.000/0001-00', 'Cidade', 'UF', TRUE)
ON CONFLICT (id) DO NOTHING;

-- Tabela legada 'lojas' (usada por migrações intermediárias antes de serem unificadas em 'empresas')
-- Necessária para que FK REFERENCES lojas(id) não falhem durante a execução do schema.
...
-- Seed básico para garantir consistência em chaves estrangeiras legadas
INSERT INTO public.lojas (id, nome_fantasia, razao_social, cnpj, cidade, uf, ativo)
VALUES ('00000000-0000-0000-0000-000000000001', 'Loja Principal', 'Razão Social Padrão', '00.000.000/0001-00', 'Cidade', 'UF', TRUE)
ON CONFLICT (id) DO NOTHING;
...
-- NOTA: Política temporária. A tabela 'perfis' ainda não foi criada neste ponto.
-- Políticas definitivas com verificação de role são recriadas após linha ~700.
...
-- Tabela de Cotas Individuais (cada bolão é dividido em N cotas)
CREATE TABLE IF NOT EXISTS public.cotas_boloes (
    id BIGSERIAL PRIMARY KEY,
    bolao_id BIGINT NOT NULL REFERENCES boloes(id) ON DELETE CASCADE,
    uid TEXT UNIQUE,
    status TEXT DEFAULT 'disponivel' CHECK (status IN ('disponivel', 'vendida', 'encalhe_casa')),
...
CREATE POLICY "cotas_boloes_read" ON cotas_boloes FOR SELECT TO authenticated USING (true);
CREATE POLICY "cotas_boloes_write" ON cotas_boloes FOR ALL TO authenticated USING (true);
...
-- NOTA: Tabela 'usuarios' não existe em bancos novos. Bloco seguro com IF EXISTS.
DO $$ BEGIN
...
-- NOTA: 'movimentacoes' e 'fechamentos_caixa' são tabelas fantasma (nunca existiram). GRANTs removidos.
...
ADD COLUMN IF NOT EXISTS validado_por_id UUID REFERENCES auth.users(id),
...
CREATE TABLE IF NOT EXISTS public.caixa_bolao_sessoes (
    ...
    validado_por_id UUID REFERENCES auth.users(id),
...
JOIN public.boloes b ON b.id = v.bolao_id
JOIN public.produtos p ON p.id = b.produto_id
LEFT JOIN public.lojas l ON l.id = b.loja_id -- Usando b.loja_id pois v.loja_id ainda não existe
